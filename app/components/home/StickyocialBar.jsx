...
const StickyocialBar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (brandsSection) {
        const brandsTop = brandsSection.offsetTop;
        const scrollPosition = window.scrollY;
        const triggerPoint = brandsTop - window.innerHeight * 0.3; // Show when 30% before brands section
        
        setIsVisible(scrollPosition > triggerPoint);
      } else {
        // Fallback: show after scrolling past first viewport
        const scrollPosition = window.scrollY;
        setIsVisible(scrollPosition > window.innerHeight * 0.9);
      }
    };
